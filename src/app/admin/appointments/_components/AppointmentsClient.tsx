...
export default function AppointmentsClient({
		...
		const fetchEmployees = async () => {
			...
			try {
				const response = await fetch(
					`/api/employees?serviceId=${selectedService}&timezone=${encodeURIComponent(timezone)}`,
					{
						credentials: "include",
					},
				);
	...
	const handleUpdateAppointment = async () => {
		...
		try {
			...
			const dateTime = parse(`${date} ${time}`, "yyyy-MM-dd HH:mm", new Date());
			// biome-ignore lint/suspicious/noGlobalIsNan: <noneed>
			...
			const response = await fetch(
				`/api/appointments/${editingAppointment.id}?timezone=${encodeURIComponent(timezone)}`,
				{
					method: "PUT",
					headers: { "Content-Type": "application/json" },
					credentials: "include",
	...
	const handleDeleteAppointment = async () => {
		...
		try {
			const response = await fetch(
				`/api/appointments/${deletingAppointmentId}`,
				{
					method: "DELETE",
					credentials: "include",
				},
			...
			setAppointments(
				appointments.filter((appt) => appt.id !== deletingAppointmentId),
			);
	...
	const handleToggleStatus = async (appointment: Appointment) => {
		...
		const newStatus =
			appointment.status === "confirmed" ? "cancelled" : "confirmed";
		try {
			const response = await fetch(
				`/api/appointments/${appointment.id}?timezone=${encodeURIComponent(timezone)}`,
				{
					method: "PUT",
					headers: { "Content-Type": "application/json" },
					credentials: "include",
